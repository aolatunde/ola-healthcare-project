...
print("RUNNING SILVER NH STATE US AVERAGES VERSION 001")
...
def write_control(status, records_read=0, records_written=0, error_message=""):
    record = [{
        ...
        "table_name": "silver_nh_stateusaverages",
...
try:
    ...
    # Standardized from: State or Nation
    df = clean_upper_string(df, "state_or_nation")

    # Standardized from: Processing Date
    df = parse_date_if_exists(df, "processing_date")

    # Cast all benchmark/metric columns to double except identifiers and dates
    non_metric_cols = {
        "state_or_nation",
        "processing_date"
    }
    ...
    dedupe_cols = [
        c for c in ["state_or_nation", "processing_date"]
        if c in df.columns
    ]
    ...
    print("Silver NH state/us averages completed")
